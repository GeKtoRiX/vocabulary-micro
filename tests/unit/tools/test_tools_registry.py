...
def test_inspect_repository_filters_files_and_reports_markers(tmp_path: Path) -> None:
    ...
    _write(tmp_path / "core" / "service.py", "VALUE = 1\n")
    ...
    assert "core/service.py" in payload["files"]
...
def test_audit_import_boundaries_reports_core_and_ui_violations(tmp_path: Path) -> None:
    _write(tmp_path / "core" / "bad.py", "import sqlite3\n")
    _write(tmp_path / "ui" / "bad.py", "import infrastructure.logging\n")
    ...
    assert any("core/bad.py" in item for item in payload["core_violations"])
    assert any("ui/bad.py" in item for item in payload["ui_violations"])
...
def test_audit_import_boundaries_falls_back_when_guardian_import_fails(
    ...
    _write(tmp_path / "core" / "ok.py", "import math\n")
    _write(tmp_path / "ui" / "ok.py", "import tkinter as tk\n")
