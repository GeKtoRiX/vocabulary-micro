...
describe('shared config owner-service storage fallbacks', () => {
  ...
  it('detects project root correctly when launched from a workspace package directory', () => {
    ...
    expect(config.lexiconDbPath).toBe(path.join(repoRoot, 'backend', 'python_services', 'infrastructure', 'persistence', 'data', 'lexicon.sqlite3'))
    expect(config.assignmentsDbPath).toBe(path.join(repoRoot, 'backend', 'python_services', 'infrastructure', 'persistence', 'data', 'assignments.db'))
  })
})
