...
from core.domain import IExportService
from infrastructure.sqlite import SqliteExportRepository, SqliteRepositoryError
...
class SqliteExcelExportService(IExportService):
    """
    Transitional export adapter.

    The implementation delegates to existing export behavior while keeping
    the new domain interface stable.
    """

    def __init__(self, db_path: Path) -> None:
        self._db_path = Path(db_path)
        self._repository = SqliteExportRepository(self._db_path)

    def export_to_excel(self, request: ExportRequest) -> ExportResult:
        resolved_db_path = self._db_path.resolve()
        ...
        if not resolved_db_path.exists():
            return ExportResult(
                success=False,
                message=f"SQLite file not found: {resolved_db_path}",
                output_path=resolved_output_path,
                stats=base_stats,
            )

        try:
            table_snapshots = self._repository.read_table_snapshots()
        except SqliteRepositoryError as exc:
            return ExportResult(
                success=False,
                message=f"Excel export failed (sqlite): {exc}",
                output_path=resolved_output_path,
                stats=base_stats,
            )
        except OSError as exc:
        ...
        snapshots_payload = [
            {
                ...
                "rows": [list(row) for row in snapshot.rows],
            }
            for snapshot in table_snapshots
        ]
        return _render_workbook_from_snapshots(
            table_snapshots=snapshots_payload,
            output_path=resolved_output_path,
            base_stats=base_stats,
        )


class HttpLexiconExportService(IExportService):
