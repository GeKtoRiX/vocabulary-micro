...
def inspect_repository(args: InspectRepositoryInput) -> dict[str, Any]:
    ...
    payload = {
        ...
        "has_core": (root / "core").exists(),
        "has_infrastructure": (root / "infrastructure").exists(),
...
class BoundaryAuditInput(BaseModel):
    ...
    Args:
        ...
        core_path: Relative path to the core package.
        ui_path: Relative path to the UI package.
        forbidden_core_roots: Import roots forbidden in `core/`.
    """

    root_path: str = Field(default=".", description="Repository root path.")
    core_path: str = Field(default="core", description="Relative core directory.")
    ui_path: str = Field(default="ui", description="Relative ui directory.")
    forbidden_core_roots: list[str] = Field(
        default_factory=lambda: ["tkinter", "sqlite3", "pandas", "infrastructure", "ui"],
        description="Top-level import roots forbidden in core.",
    )
...
def audit_import_boundaries(args: BoundaryAuditInput) -> dict[str, Any]:
    """Audit core/ui import boundaries via static AST inspection.
...
TOOL_REGISTRY: dict[str, ToolDefinition] = {
    ...
    "audit_import_boundaries": ToolDefinition(
        ...
        description="Validate import boundaries for core/ui layers.",
    ),
